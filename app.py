...
PAGE_TITLE = "Allegheny County Benefits Navigator"
TRI_STATE_OPTIONS = {"Unknown": None, "Yes": True, "No": False}
CLEARABLE_SESSION_KEYS = [
    "selected_case",
    "user_description",
    "county",
    "zip_code",
...
def _clear_current_screening() -> None:
    defaults: dict[str, Any] = {
        ...
        "food_insecurity_signal": "",
        "language_or_stress_notes": "",
        "child_under_5": "Unknown",
        "pregnant_household_member": "Unknown",
        "elderly_or_disabled_member": "Unknown",
        "heating_assistance_need": "Unknown",
        "recent_job_loss": "Unknown",
        "chat_active": False,
        "chat_history": [],
        "chat_raw_input": {},
        "chat_rounds": 0,
    ...
    for key in CLEARABLE_SESSION_KEYS:
...
def _render_results(session) -> None:
    ...
    with st.expander("Reviewer trace"):
        st.caption("Compact reviewer trace for this screening run. It is session-local and is cleared by 'Clear current screening'.")
        st.json(build_reviewer_trace(session, case_id=st.session_state.get("selected_case")))
...
def main() -> None:
    ...
    st.title(PAGE_TITLE)
    st.caption(
        "Benefits prescreening for Allegheny County households using structured intake, policy retrieval, and conservative recommendation checks."
    )
    st.info(
        "This tool is for benefits prescreening, not an official eligibility decision. Information you enter may remain visible in this browser session until you clear it or refresh or close the session. Use 'Clear current screening' to remove household information and results from this session. Avoid entering information you do not want processed by the tool."
    )
    ...
    with st.sidebar:
        ...
        st.caption(
            "Clearing the current screening removes the household description, structured inputs, intake conversation, results, retrieved evidence, and session JSON from this browser session."
        )
        if st.button("Clear current screening", use_container_width=True):
    ...
    with st.container():
        ...
        with st.form("navigator_form"):
            ...
            submitted = st.form_submit_button("Run Benefits Navigator", use_container_width=True)
        ...
        if st.session_state.get("nav_session"):
            ...
            if nav.uploaded_documents_available:
                ...
                st.caption(
                    "Uploaded policy files shown here are app-available policy sources and are not deleted by clearing the current screening session."
                )
            _render_results(nav)
