...
def run_eligibility_and_prioritization(
    ...
    *,
    ...
    program_matches = [_apply_priority_adjustments(match, profile) for match in program_matches]
    recommended = [
        match
        for match in program_matches
        if match.decision_state in {"likely_eligible", "possibly_eligible"}
    ]
    recommended = _sort_review_candidates(recommended)

    for match in program_matches:
        if match.decision_state in {
            "needs_clarification",
            "suppressed_high_income",
            "suppressed_out_of_scope",
            "suppressed_insufficient_evidence",
        }:
            uncertainty_flags.append(
                f"{match.program_name}: {match.cross_check_summary or match.suppression_reason or match.decision_state}."
            )

    decision_status = _determine_decision_status(
        intake_status,
        profile,
        missing_fields,
        contradictory_fields,
...
{{
  ...
  "missing_info": [<information that would change this assessment>]
    ...
    return ProgramMatch(
        ...
        llm_match_score=match_score,
        cross_check_status="live_llm_cross_check_used",
        cross_check_summary="Live LLM review used retrieved policy text to assess this program.",
...
def _rule_based_evaluation(
    ...
    retrieval_cap = 1.5 if positive_signal_count > 0 else 0.5
    retrieval_bonus = round(min(sum(chunk.score for chunk in retrieved[:3]), retrieval_cap), 3)
    if retrieved:
    ...
    return ProgramMatch(
        ...
        retrieved_evidence=retrieved,
        checklist_items=checklist_items,
        suppression_reason="high_income" if over_income else None,
        rule_match_score=round(score, 3),
...
def _evaluate_uploaded_program(
    ...
    return ProgramMatch(
        ...
        rule_match_score=round(score, 3),
        llm_match_score=None,
        cross_check_status="uploaded_policy_rule_only",
        cross_check_summary="Uploaded-policy retrieval produced a provisional rule-only result.",
...
def _cross_check(llm: ProgramMatch, rules: ProgramMatch) -> ProgramMatch:
    ...
    cross_reason = MatchReason(
        reason=(
            ...
            + ("Both methods agree." if gap == 0 else f"LLM says {llm.status}; rules say {rules.status}.")
        ),
        score=rules.match_score or 0.0,
        evidence_snippet=None,
    )

    if rule_rank == 0 and llm_rank > 0:
        caveats = _dedupe(
            list(rules.caveats)
            + [
                "Deterministic screening did not support a normal recommendation, so the LLM-positive result was withheld."
            ]
        )
        return rules.model_copy(
            update={
                "rationale": list(rules.rationale)[:5] + [cross_reason],
                "caveats": caveats,
                "llm_match_score": llm.match_score,
                "rule_match_score": rules.match_score,
                "cross_check_status": "suppressed_rule_conflict",
                "cross_check_summary": (
                    "LLM suggested this program, but deterministic screening did not support a normal recommendation."
                ),
                "llm_rule_conflict": True,
            }
        )

    final_match = llm if llm_rank <= rule_rank else rules
    caveats = list(final_match.caveats)
    if gap:
        caveats.append(
            f"Cross-check difference noted: LLM suggested {llm.status} while deterministic screening suggested {rules.status}."
        )

    return final_match.model_copy(
        update={
            "rationale": list(final_match.rationale)[:5] + [cross_reason],
            "caveats": _dedupe(caveats),
            "llm_match_score": llm.match_score,
            "rule_match_score": rules.match_score,
            "cross_check_status": "live_llm_cross_check_used" if gap == 0 else "cross_check_disagreement",
            "cross_check_summary": (
                "LLM and deterministic screening aligned."
                if gap == 0
                else f"LLM suggested {llm.status} while deterministic screening suggested {rules.status}."
            ),
            "llm_rule_conflict": gap > 0,
        }
    )
...
def _status_from_score(
    ...
    *,
    ...
    if not retrieved and score < 4:
        return "no_clear_match"
    if profile.household_income_total is None:
        return "possible_match" if positive_signal_count >= 2 else "no_clear_match"
    if profile.employment_status == "full_time" and profile.monthly_earned_income == 0:
    ...
    if positive_signal_count == 0 and score < 2:
        return "no_clear_match"
    if score >= 6:
        return "strong_match"
    if score >= 3:
...
def _finalize_program_match(
    ...
    if _is_out_of_scope(profile):
        ...
        suppression_reason = "contradictory_intake"
        cross_check_summary = "Conflicting household details need clarification before normal recommendations can be shown."
    elif _needs_program_clarification(match.program_name, intake_status, missing_fields):
        decision_state = "needs_clarification"
    ...
    elif match.suppression_reason == "high_income":
    ...
    elif match.cross_check_status == "suppressed_rule_conflict":
        decision_state = "suppressed_insufficient_evidence"
    elif match.status == "strong_match":
    ...
    if decision_state not in {"likely_eligible", "possibly_eligible"}:
        status = "no_clear_match" if decision_state != "unsupported_or_unknown" else match.status
        priority_score = min(priority_score, max(match.match_score, 0.0))
...
def _needs_program_clarification(program_name: str, intake_status: str, missing_fields: list[str]) -> bool:
    ...
    if "county" in missing_fields:
        return True
    return "num_children" in missing_fields and program_name == "Medicaid/CHIP"


def _apply_priority_adjustments(match: ProgramMatch, profile: UserIntake) -> ProgramMatch:
    if match.decision_state not in {"likely_eligible", "possibly_eligible", "needs_clarification"}:
        return match

    boost = 0.0
    reason = match.priority_boost_reason
    if match.program_name == "Medicaid/CHIP" and profile.pregnant_household_member:
        boost = 4.0
        reason = "Pregnancy-related coverage should be reviewed first."
    elif (
        match.program_name == "Medicaid/CHIP"
        and (profile.num_children or 0) > 0
        and profile.insurance_status == "uninsured"
    ):
        boost = 4.0
        reason = "Children without insurance should be reviewed for Medicaid/CHIP first."

    if boost == 0.0:
        return match

    rationale = list(match.rationale)
    rationale.append(MatchReason(reason=reason, score=boost))
    return match.model_copy(
        update={
            "priority_score": round(match.priority_score + boost, 3),
            "priority_boost_reason": reason,
...
def _determine_decision_status(
    ...
    missing_fields: list[str],
    contradictory_fields: list[str],
    recommended: list[ProgramMatch],
) -> str:
    if _is_out_of_scope(profile) or contradictory_fields:
        return "ambiguous"
    if intake_status != "complete" or missing_fields:
        return "ambiguous"
    if not recommended:
        return "ready_for_explanation"
    if any(match.decision_state == "possibly_eligible" for match in recommended):
        return "ambiguous"
    return "ready_for_explanation"
