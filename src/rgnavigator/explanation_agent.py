...
def _build_next_steps(intake: IntakeOutput, eligibility: EligibilityOutput) -> list[str]:
    ...
    if eligibility.recommended_programs:
        ...
        steps.append("No likely program match was identified from the current information.")
        steps.append("Check COMPASS, Allegheny County DHS, or a human benefits counselor if you want an official follow-up.")
    ...
    if intake.missing_fields:
        ...
        if "household_income_total" in intake.missing_fields:
            steps.append("Monthly household income is especially important for income-tested programs.")
    if intake.contradictory_fields:
...
def _build_explanation(intake: IntakeOutput, matches, eligibility: EligibilityOutput) -> str:
    ...
    if not matches:
        suppression_matches = [
            match
            for match in eligibility.program_matches
            if match.decision_state.startswith("suppressed_")
        ]
        if intake.missing_fields:
            return (
        ...
        if any(match.decision_state == "suppressed_high_income" for match in suppression_matches):
            return (
                f"{intro} Based on the information provided, the system did not identify a likely benefit match. "
                "The main reason is that the reported household income appears above the screening bands for the programs reviewed. "
                "This is not an official denial, and benefits rules can still be complex, so you may want to verify with COMPASS, Allegheny County DHS, or a human benefits counselor."
            )
        if any(match.decision_state == "suppressed_insufficient_evidence" for match in suppression_matches):
            return (
                f"{intro} Based on the information provided, the system did not identify a likely benefit match it could safely recommend. "
                "Retrieved policy evidence or cross-check signals did not support a normal recommendation strongly enough, so the system is staying conservative. "
                "You may still want to verify with official program tools or a human benefits counselor."
            )
        return (
            f"{intro} Based on the information currently available, the system did not identify a likely benefit match. "
            "That does not mean the household is officially ineligible; it means the current profile and retrieved evidence did not support a normal recommendation strongly enough."
        )
    ...
    for match in matches:
        ...
        if match.priority_boost_reason:
            pieces.append(f"{match.program_name} is prioritized here because {match.priority_boost_reason.lower()}")
        if match.retrieved_evidence:
            top_chunk = match.retrieved_evidence[0]
...
def _is_out_of_scope(intake: IntakeOutput) -> bool:
    return intake.geography_status == "out_of_scope_geography"
